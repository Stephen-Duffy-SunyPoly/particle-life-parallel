...
void wcstrTocstr(wchar_t * in, LPSTR out) {
	std::string cs = convertWideToNarrow(in);
	memcpy( out, cs.c_str(), cs.length() );
	out[cs.length()] = '\0';
}

void wcstrTocstr2(wchar_t * in, LPCSTR out) {
	wcstrTocstr(in, const_cast<LPSTR>(out));
}
...
ofFileDialogResult ofSystemLoadDialog(std::string windowTitle, bool bFolderSelection, std::string defaultPath){
	...
	if (bFolderSelection == false){
		...
		if(defaultPath!=""){
			...
			wcstrTocstr2(szDir,ofn.lpstrInitialDir);
		}

		if (windowTitle != "") {
			wcscpy(szTitle, convertNarrowToWide(windowTitle).c_str());
			wcstrTocstr2(szTitle,ofn.lpstrTitle);
		} else {
			ofn.lpstrTitle = nullptr;
		}

		ofn.lpstrFilter = "All\0";
		wcstrTocstr(szFileName,ofn.lpstrFile);
